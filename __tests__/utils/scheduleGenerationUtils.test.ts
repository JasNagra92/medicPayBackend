...
describe("generateSingleDaysDataForClient", () => {
  ...
  it("testing super stat on Jan 1", () => {
    let userInfoFromRequest: IUserDataForDB = {
      id: "test",
      email: "test",
      shiftPattern: "Alpha",
...
describe("generateRegularOTShift", () => {
  ...
  it("regular OT worked on a stat holiday should have the day data filled in with OTStat filled as the property", () => {
    const testUserInfo: IUserDataForDB = {
      id: "3r342edsfserseresdf",
      email: "test",
      shiftPattern: "Alpha",
    ...
    // Dec 26th 2023 for D platoon is 3nd day off
    const dateOfShift = new Date(2023, 11, 25);
    // 12 hour shift worked on Dec 26th a stat holiday
    const shiftStart = new Date(2023, 11, 25, 6);
    const shiftEnd = new Date(2023, 11, 25, 18);
    const OTShiftAlpha = "Alpha";
    let result = generateRegularOTShift(
      testUserInfo,
      dateOfShift,
    ...
    expect(result.OTSuperStat).toEqual(12);
  });
});
